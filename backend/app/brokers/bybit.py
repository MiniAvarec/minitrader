"""Async Bybit v5 linear (USDT-margined) perpetual broker (ccxt + ccxt.pro)."""
...
log = logging.getLogger("brokers.bybit")
...
class BybitBroker(Broker):
    ...
    async def usdt_balance(self) -> float:
        bal = await self.client.fetch_balance({"type": "swap"})
    ...
    async def fetch_realized_pnl(self, symbol: str, since_ms: int) -> float:
        # Bybit v5 closed-pnl: GET /v5/position/closed-pnl
        params = {"category": "linear", "symbol": symbol, "startTime": since_ms}
        rows = await self.client.privateGetV5PositionClosedPnl(params)
        items = (rows.get("result") or {}).get("list") or []
        return float(sum(float(r.get("closedPnl") or 0.0) for r in items))
    ...
    async def load_exchange_info(self) -> list[InstrumentInfo]:
        ...
        for ccxt_sym, m in markets.items():
            if not (m.get("linear") and m.get("swap")):
                continue
            if m.get("quote") != "USDT":
                continue
            native = m.get("id") or ccxt_sym  # e.g. "BTCUSDT"
    ...
    def _ensure_pro(self) -> None:
        ...
        if self._pro_client is None:
            self._pro_client = ccxt_pro.bybit(
                {
                    "apiKey": self.api_key,
    ...
    async def iter_user_data(self) -> AsyncIterator[FillEvent]:
        ...
        while True:
            try:
                ...
                for o in orders:
                    yield _ccxt_order_to_fill("bybit", o)
            except Exception as e:
                log.warning("Bybit watch_orders error: %s; reconnect in 3s", e)
    ...
    async def iter_klines(
        ...
        async def _watch(native: str, tf: str) -> None:
            ccxt_sym = to_ccxt_symbol("bybit", native)
            while True:
                try:
                    ...
                    log.warning("Bybit watch_ohlcv %s %s error: %s", native, tf, e)
...
def _ccxt_order_to_fill(exchange: str, o: dict) -> FillEvent:
    ...
    info = o.get("info") or {}
    realized = float(info.get("closedPnl") or info.get("cumExecFee") or 0.0)
    return FillEvent(
        exchange=exchange,
